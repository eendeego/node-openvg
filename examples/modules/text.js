...
var drawText = text.drawText = function(x, y, s, f, pointsize) {
  var size = pointsize, mm = new Float32Array(9);

  openVG.getMatrix(mm);

  openVG.translate(x, y);
  openVG.scale(size, size);
  ...
  for (var i = 0; i < s.length; i++) {
    ...
    openVG.translate(f.glyphAdvances[glyph] / 65536.0, 0);
  }
...
var loadFont = text.loadFont = function(name) {
  ...
  for (var i = 0; i < jsonf.glyphCount; i++) {
    ...
    var path = openVG.createPath(openVG.VG_PATH_FORMAT_STANDARD,
    ...
    if (instructionCounts) {
      openVG.appendPathDataO(path, instructionCounts,
                             glyphInstructions, jsonf.glyphInstructionIndices[i],
                             glyphPoints, 4 * 2 * jsonf.glyphPointIndices[i]);
    }
  }
  return f;
}
